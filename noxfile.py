...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/servicenow/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/servicenow/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/servicenow/*"
            )
            ...
            #    "--include=src/saltext/servicenow/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/servicenow/config/schemas",
    )
